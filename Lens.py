...
def place_eye(frame, eyeCenter, eyeSize):  
   eyeSize = int(eyeSize//2)-8
   x1 = int(eyeCenter[0,0] - (eyeSize/2)-2)  
   x2 = int(eyeCenter[0,0] + (eyeSize/2)+2)  
   y1 = int(eyeCenter[0,1] - (eyeSize/2)-2)  
   y2 = int(eyeCenter[0,1] + (eyeSize/2)+2)  
...
imgEye = cv2.imread('eyes/2.png',-1)  
imgEye = cv2.GaussianBlur(imgEye, (5,5), cv2.BORDER_DEFAULT)
